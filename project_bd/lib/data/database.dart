...
import 'package:project_bd/Model/cart.dart';
...
class DatabaseHelper {
  ...
  void _onCreate(Database db, int version) async {
    ...
    await db.execute(
        'CREATE TABLE Carrinho(idPrato INTEGER, quant INTEGER)');
...
//insertion
  ...
  Future<bool> saveCart(Cart c) async {
    var dbClient = await db;
    try {
      await dbClient.rawInsert(
  ...
  Future<Map<Prato, int>> getCart() async {
    var dbClient = await db;
    List<Map<String, dynamic>> idsPratos = await dbClient.rawQuery('SELECT idPrato FROM Carrinho');
    Map<Prato, int> list = Map<Prato, int>();

    print("\n\n\n IIIII   ");

    print("\n\n\n Ids e Lista com itens   ");

    for(Map<String, dynamic> m in idsPratos){
      print(m.values);
      for (int i in m.values) {
        dynamic test = await dbClient.rawQuery(
        '''
        SELECT 
              Prato.*,
        ...
        WHERE 
              idPrato=?''', [i]);
        dynamic quant = await dbClient.rawQuery('SELECT quant FROM Carrinho WHERE idPrato=?', [i]);
      try {
        print("Quant: ${quant[0]['quant']}");
        print("Quant: $quant[0]");
        list.addAll({Prato.map(test[0]): quant[0]['quant']});
      } catch (e) {
        print(e.toString());
      }
    }
